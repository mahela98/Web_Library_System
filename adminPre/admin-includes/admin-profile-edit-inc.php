...
if (isset($_POST["submit"])) {
    ...
    function editUser($conn,$userID,$fullName,$userName,$userEmail,$mobile){
        ...
        if (mysqli_query($conn, $sql)) {
            header('location:../admin-profile-edit.php?message='.$userId.' &error=usereditedsuccessfuly');
    ...
    if (emptyUserInput($userName,$fullName)) {
        header('location:../admin-profile-edit.php?message='.$userId.' &error=emptyInputbookNameinedit');
        exit();
    }
...
header("location:../admin-profile-edit.php?error=notworkinguserEdit");
    exit();
}
}
