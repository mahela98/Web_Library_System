...
$checkpwd=password_verify($currentPassword,$hashPassword);
    if (!$checkpwd) {
        echo "wrong";
        header ("location:../admin-profile-view.php?error=errorInChangingPWEnteredPWIsWrong");
        exit (); 
    }
    else if ($checkpwd) {
        ...
        function changePW($conn,$userID,$newPassword){
            ...
            if (mysqli_query($conn, $sql)) {
                header('location:../admin-profile-view.php?error=passwordeChangedSuccessfuly'); 
            } 
            else {
